      ...
      steps {
        container('cdt') {
          timeout(activity: true, time: 20) {
            ...
            sh 'git config remote.origin.url https://github.com/jonahgraham/cdt.git'
            /* Workaround for Bug 568904 */
            sh 'git config protocol.version 1'
            sh 'git fetch --no-tags --force --progress -- https://github.com/jonahgraham/cdt.git +refs/heads/*:refs/remotes/origin/*'
          }
          checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'CheckoutOption', timeout: 20], [$class: 'CloneOption', depth: 0, noTags: true, reference: '', shallow: false, timeout: 20]], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/jonahgraham/cdt.git']]])
        }
      }
    }
    stage('Run build') {
